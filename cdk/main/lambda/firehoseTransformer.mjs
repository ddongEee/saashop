 ...
 *
 */
function transformLogEvent(logEvent, timestamp, logGroupName) {
  try {
    const logData = JSON.parse(logEvent.message);
    logData.firehose_arrival_time = new Date(timestamp);
...
function processRecords(records) {
  return records.map((r) => {
    ...
    const firehose_arrival_time = r.approximateArrivalTimestamp;
    const logGroupName = data.logGroup;
    ...
    if (data.messageType === 'CONTROL_MESSAGE') {
      ...
      const joinedData = data.logEvents.map((e) => transformLogEvent(e, firehose_arrival_time, logGroupName)).join('');
      const encodedData = Buffer.from(joinedData, 'utf8').toString('base64');
      return {
        data: encodedData,
        result: 'Ok',
        recordId: recId,
...
function splitCWLRecord(cwlRecord) {
  ...
  return [rec1, rec2].map((r) => zlib.gzipSync(Buffer.from(JSON.stringify(r), 'utf8')));
}
...
async function putRecordsBase(
  ...
  if (failed.length > 0) {
    if (attemptsMade + 1 < maxAttempts) {
      console.log(`Some records failed while calling ${commandClass}, retrying. ${errMsg}`);
...
export const handler = async function (event, context) {
  ...
  if (recordListsToReingest.length > 0) {
    ...
    const client = isSas ? new KinesisClient(clientArgs) : new FirehoseClient(clientArgs);
